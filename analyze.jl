...
function rand_pair_cor_e(bin, lt, lr, Neurons, n, error_code)
  if Neurons == -5
    return -5
  elseif length(lt) < 1
    return error_code
...
function count_train_intron_e(bin, lt, lr, Neurons, n, shuff, error_code)
  if length(lt) < 1
    return error_code
  end
  if shuff == true
...
function read_thresholds(filename)
  x = open(filename);
  a = read(x, String);
  b = split(a, "\n");
  c = split(b[1], ", ")
  et = parse(Float64, c[1])
  it = parse(Float64, c[2])
  close(filename)
  return et, it
end
...
function read_inputs(filename)
  ...
  b = split(a, ",");
  b = b[1:end-1];
  c = [parse(Float64, b[i]) for i in eachindex(b)]
...
function weights_lookup(i, raster, W, compartments, index_increment)
  ...
  if length(kx) > 0
    for j = 1:4
      ws = sum(W[compartments[j]:compartments[j] + index_increment, i])
      output = ws * length(kx)
      outs[j] = output
